...
export class QuotaService {
  ...
  public async getQuotaByType(userId: number, quotaType: string) {
    ...
    if (!quota) {
      ...
      quota.isResettable &&
      this.shouldResetQuota(quota.resetFrequency, quota.lastResetTimestamp)
    ) {
  ...
  public async incrementQuota(
    userId: number,
    quotaType: string,
    increment: number,
  ) {
    const quota = await this.getQuotaByType(userId, quotaType);
